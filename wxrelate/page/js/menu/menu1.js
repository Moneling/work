...
//初始化数据库
var menuList ={items:[1]}
$.ajax({
	    type: "POST",
	    url:'/woplat/management/publicmenu!query.action',
	    data:{node:"module-0"},
	    dataType:"json",
	    async:false,
...
function setSecondmenuPostion(){
	...
	var secendmenus = document.querySelectorAll(".secend-menu");
		...
		for(var m=0;m<menuItems.length;m++){
			...
			(function(m){
			menuItems[m].onclick=function(){
				var ite=menuList.items;
				var sid= ite[m].id;
				$.ajax({
			    type: "POST",
			    url:'/woplat/management/publicmenu!query.action',
...
//					]
					...
					var liStr='';
					var addSecond='<li><a href="javascript:void(0);"  onclick="addSecondmenu(this)" class="addSecondmenu">+</a></li>';
					if(menuListchild.length<5){
						for(var i=0;i<menuListchild.length;i++){
						liStr +=`<li><a href="javascript:void(0);"  onclick="addSecondmenu(this)" class="addSecondmenu">${menuListchild[i].text}</a></li>`;
						}
						liStr+=addSecond;
					}else {
						for(var i=0;i<menuListchild.length;i++){
						liStr +=`<li><a href="javascript:void(0);"  class="addSecondmenu">${menuListchild[i].text}</a></li>`;
						}
					}
		...
		/*添加二级菜单*/
$(function(){
	 $(".addSecondmenu").on("click",function(){
	 	alert("1");
	 	return false;
	 })
})
